...
pub(crate) fn spawn_first_user_process() -> Result<ProcessId, ProcessError> {
    ...
    let owner_cpu = crate::cpu::current_id().map_err(|_| ProcessError::InvalidProcess)?;
    let pid = allocate_process_slot(None, FdTable::new(), cwd_dir, owner_cpu)?;
    let mut staged_address_space = Some(vm::create_user_address_space().map_err(|err| {
    ...
    let result = (|| {
        let aspace = staged_address_space
            .as_mut()
        ...
        );
        {
            ...
            let main_thread = sched::thread_spawn_user(
                ...
                initial_sp,
                0,
                ThreadAttrs::joinable().with_affinity(owner_cpu),
                |thread, home_cpu, address_space| {
                    bind_published_user_thread(&mut table, pid, thread, home_cpu, address_space);
                },
            )
    ...
    if result.is_err() {
        ...
        drop(stack);
        if let Some(address_space) = attached.address_space {
        ...
        } else if let Some(address_space) = staged_address_space {
